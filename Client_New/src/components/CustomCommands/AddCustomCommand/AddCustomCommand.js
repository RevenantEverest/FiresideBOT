...
class AddCustomCommand extends Component {
    ...
    handleSubmit(e) {
        ...
        let data = {
            ...
            created_by: this.props.userData.discord_id
        };

        if(!data.input || !data.output)
            return this.toggleFailureNotify("No empty fields");

        if(this.state.defaultCommands.map(el => el.name).includes(data.input.toLowerCase()))
            return this.toggleFailureNotify("Command Input is already a Fireside command");

        ...
        if(results.data.data) {
          ...
          for(let i = 0; i < this.state.discordGuilds.length; i++) {
            if(value === this.state.discordGuilds[i].id) return this.setState({ chosenGuild: this.state.discordGuilds[i] }, () => {
              ...
              window.location = `https://discordapp.com/api/oauth2/authorize?client_id=441338104545017878&response_type=code&guild_id=${this.state.chosenGuild.id}&permissions=2146958583&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Fuser%2Fdashboard&scope=bot`
            })
          }
        }
      })

...
function getRoute(baseEndpoint: string, app: Application, authPayload: AuthTestingPayload, extraParams: GuildSongExtraParams) {
    describe("given the user is not logged in", () => {
        it("should return a 403 status", async () => {
            await supertest(app)
            .get(`${baseEndpoint}/guild_id/${extraParams.guildId}/songs`)
            .expect(403)

            await supertest(app)
            .get(`${baseEndpoint}/guild_id/${extraParams.guildId}/id/1/songs`)
            .expect(403)

            await supertest(app)
            .get(`${baseEndpoint}/guild_id/${extraParams.guildId}/id/1/songs/id/1`)
            .expect(403)
        });
    });
        ...
        describe("given the user is a guild member", () => {
            describe("given the guild id and playlist id as a param", () => {
                ...
                describe("given the song id as a param", () => {
                    describe("given the song id doesn't exist", () => {
                        it("should return a 404 status", async () => {
                            ...
                            const endpoint = `${baseEndpoint}/guild_id/${extraParams.guildId}/id/1/songs/918273`;
                            await supertest(app)
                            .get(endpoint)
                            .set(authPayload.header)
                            .expect(404)
                    ...
                    describe("given the song id exists", () => {
                        it("should return the guild song", async () => {
                            ...
                            const endpoint = `${baseEndpoint}/guild_id/${extraParams.guildId}/id/1/songs/id/${extraParams.createdSong?.id}`;
                            const { body, statusCode } = await supertest(app)
                            .get(endpoint)
                            .set(authPayload.header)
                            .send()
                ...
                it("should return paginated songs", async () => {
                    ...
                    const endpoint = `${baseEndpoint}/guild_id/${extraParams.guildId}/id/1/songs`;
                    const { body, statusCode } = await supertest(app)
                    .get(endpoint)
                    .set(authPayload.header)
                    .send()

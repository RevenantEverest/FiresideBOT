...
const baseEndpoint = "/custom/fortunes";

const extraParams: FortuneExtraParams = {
    guildId: DISCORD.TESTING_SERVER_ID,
    mocks: { hasPermission, isGuildMember }
};

describe("Fortunes Route", () => {
    beforeAll(async () => {
    ...
    //     destroyRouteSpec(baseEndpoint, app, authPayload, extraParams);
    // });
});
const db = require('../../models/UserModels/userSongsDB');
...
services.index = async (bot, message, command, data, callback, qrecCallback) => {
    db.findAll()
...
services.save = async (bot, message, command, data, callback) => {
    db.save(data)
    .then(song => callback(song))
    .catch(err => errorHandler(bot, message, err, "Error Saving Song", command));
};

services.delete = async (bot, message, command, data, callback) => {
    db.destroy(data)
    .then(song => callback(song))
    .catch(err => errorHandler(bot, message, err, "Error Deleting Song", command));
};
